...
def test_absolute_asset_url2():
    ...
    assert (
        html.absolute_asset_url("", None, foo="bar", hello="world")
        == "/?hello=world&foo=bar"
    )
    assert (
        html.absolute_asset_url("abc", "/static/root", foo="bar")
        == "/static/root/abc?foo=bar"
    )
    assert (
...
def test_absolute_asset_url6():
    ...
    assert (
        h.absolute_asset_url("http://localhost/tmp?a=42", "/abc")
        == "http://localhost/tmp?a=42"
    )

    assert h.absolute_asset_url("/tmp?a=42", "/abc", b=43) in (
        "/tmp?a=42&b=43",
        "/tmp?b=43&a=42",
    )
    assert h.absolute_asset_url("tmp?a=42", "/abc", b=43) in (
