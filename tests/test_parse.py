...
def test_parse3():
    ...
    with pytest.raises(XMLSyntaxError):
        h.fromfile(filename, encoding='utf-8', recover=False)

    h.fromfile(filename, encoding='iso8859-1', recover=False)
...
def test_parse5():
    ...
    assert root.tostring() == b'<html><body>test</body></html>'


def test_parse6():
    h = html.Renderer()
    root = h.fromstring('<a>text</a>')
    assert type(root) is html.Tag
