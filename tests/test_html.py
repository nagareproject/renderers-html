...
def test_url():
    ...
    assert (
        h.img(src="/abc", lowsrc="/def").tostring() == b'<img src="/abc" lowsrc="/def">'
    )
    assert (
        h.img(src="abc", lowsrc="def").tostring() == b'<img src="/abc" lowsrc="/def">'
    )
    ...
    assert (
        h.img(src="abc", lowsrc="def").tostring()
        == b'<img src="/root/abc" lowsrc="/root/def">'
    )
...
def test_cache_buster():
    ...
    assert h.script(src="/abc").tostring() == b'<script src="/abc"></script>'
    assert (
        h.script(src="abc").tostring() == b'<script src="/root/abc?ver=1.2"></script>'
    )

    h = html.Renderer(static_url="/root", assets_version="1.2")

    assert (
        h.script(src="/abc?foo=bar").tostring()
